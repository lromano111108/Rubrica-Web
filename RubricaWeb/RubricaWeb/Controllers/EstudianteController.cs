    ...
    public class EstudianteController : Controller
    {
        ...
        public ActionResult CargaEstudiante(Estudiante model)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                int idEstudiante = AD_Estudiante.ultimoIdEstudiante();
                if (resultado)
                {
                    List<Materia> listadoId = AD_Estudiante.ListadoIdMateria(model.IdCurso);

                    foreach (var item in listadoId)
                    {
                        AD_Estudiante.AgregarEstudianteXMateria(idEstudiante, item.IdMateria);
                        List<VM_Tema> listadoIdTema = AD_Estudiante.ListadoTemasPorMateria(item.IdMateria);

                        foreach (var valoracion in listadoIdTema)
                        {
                            AD_Estudiante.AgregarEstudianteAValoracion(idEstudiante, valoracion.IdTema);
                        }
                    }
